  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
      ...
      <div className="text-left space-y-2">
        <span className="text-xs uppercase tracking-[0.25em] text-brand-sage font-bold block">Vicesh Cosmetics Desk</span>
        <h1 className="font-editorial text-3xl sm:text-4xl font-bold text-brand-forest">Contact Our Team</h1>
      ...
      <div className="bg-brand-forest text-brand-cream border border-brand-gold/25 rounded-md p-6 sm:p-8 text-left grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        ...
        <div className="md:col-span-4 flex flex-col sm:flex-row md:flex-col gap-2.5">
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className="w-full text-center px-5 py-3 bg-brand-gold hover:bg-brand-cream hover:text-brand-forest text-brand-forest text-xs font-bold uppercase tracking-wider rounded-xs transition-all shadow-xs"
          >
            Wholesale WhatsApp
          </a>
          <a 
            href="mailto:viceshcompanyltd@gmail.com?subject=Wholesale%20Inquiry"
            className="w-full text-center px-5 py-3 border border-brand-cream hover:bg-brand-cream hover:text-brand-forest text-brand-cream text-xs font-bold uppercase tracking-wider rounded-xs transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-5 space-y-8 text-left">
          ...
          <div className="space-y-4">
            <h3 className="font-editorial text-2xl font-bold text-brand-forest">Corporate Office & Showroom</h3>
          ...
          {/* Quick contact list */}
          <div className="space-y-4 text-xs font-light text-brand-sage leading-relaxed">
            
            <div className="flex gap-3 items-start bg-brand-offwhite border border-brand-beige/25 p-4 rounded-xs">
              <MapPin className="w-5 h-5 text-brand-sage shrink-0" />
              <div>
                <p className="font-bold text-brand-forest uppercase tracking-wider text-[9px] mb-0.5">Physical Headquarters Address</p>
                <p className="font-semibold text-brand-forest">CL-0635-9738, Oguaakrom, Winneba Road, Ghana.</p>
                <p className="mt-1 text-[10px] text-brand-gold font-medium">Pickup spot available at: Accra, Makola</p>
              </div>
            </div>

            <div className="flex gap-3 items-start bg-brand-offwhite border border-brand-beige/25 p-4 rounded-xs">
              <Phone className="w-5 h-5 text-brand-sage shrink-0" />
              <div>
                <p className="font-bold text-brand-forest uppercase tracking-wider text-[9px] mb-0.5">Phone & WhatsApp Support Desk</p>
                <p className="font-semibold text-brand-forest text-sm">0595780477</p>
                <p className="font-semibold text-brand-forest mt-0.5">+233 (0) 59 578 0477</p>
                <p className="mt-1">Available Monday to Saturday for direct orders and tele-consultation.</p>
              </div>
            </div>

            <div className="flex gap-3 items-start bg-brand-offwhite border border-brand-beige/25 p-4 rounded-xs">
              <Mail className="w-5 h-5 text-brand-sage shrink-0" />
              <div>
                <p className="font-bold text-brand-forest uppercase tracking-wider text-[9px] mb-0.5">Email Support Inbox</p>
                <p className="font-semibold text-brand-forest">viceshcompanyltd@gmail.com</p>
                <p className="mt-1">For sales inquiries, corporate collaborations, or order queries.</p>
              </div>
            </div>

            <div className="flex gap-3 items-start bg-brand-offwhite border border-brand-beige/25 p-4 rounded-xs">
              <Clock className="w-5 h-5 text-brand-sage shrink-0" />
              <div>
                <p className="font-bold text-brand-forest uppercase tracking-wider text-[9px] mb-0.5">Support Desk Timing</p>
          ...
          <div className="relative overflow-hidden bg-brand-forest text-brand-cream border border-brand-gold/25 rounded-md p-6 space-y-4 shadow-sm">
            ...
            <div className="bg-black/15 p-3 rounded-xs border border-brand-gold/10 z-10 relative text-[11px] leading-relaxed">
              ...
              <p className="text-brand-cream/80 font-light">
                If additional brand guidance is required, or design improvements are suggested, stakeholders and team partners should contact the <span className="font-semibold text-white">VICESH Communications Manager</span> at <span className="font-semibold text-brand-gold">brand@vicesh.com</span> or via this page.
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 bg-brand-offwhite border border-brand-beige/25 rounded-md p-6 sm:p-10 shadow-xs text-left space-y-6">
          ...
          {submitted ? (
            <div className="bg-brand-sage/10 border border-brand-sage/35 text-center p-8 rounded-xs space-y-3">
              ...
              <h4 className="font-editorial text-xl font-bold text-brand-forest">Ticket Submitted Successfully!</h4>
              ...
              <button 
                ...
                className="text-xs font-bold uppercase tracking-widest text-brand-forest hover:text-brand-sage pt-2 underline cursor-pointer"
              >
                Send Another Ticket
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs text-brand-forest font-semibold block">Your Name</label>
                  <input
                    type="text"
                    value={name}
              ...
              {/* Subject */}
              <div className="space-y-1">
                <label className="text-xs text-brand-forest font-semibold block">Inquiry Topic</label>
                <select
                  value={subject}
              ...
              {/* Message */}
              <div className="space-y-1">
                <label className="text-xs text-brand-forest font-semibold block">Message Details</label>
                <textarea
                  rows={6}
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder="Tell us what you need..."
                  className="w-full text-xs bg-brand-cream border border-brand-beige/65 px-4 py-3 rounded-xs focus:outline-none focus:border-brand-sage text-brand-forest resize-none"
                  required
                />
              </div>
